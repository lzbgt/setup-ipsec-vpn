...
YOUR_IPSEC_PSK='Hz123456'
YOUR_USERNAME='luzb'
YOUR_PASSWORD='Hz123456'
...
apt-get -yq update || yum update -y || exiterr "'apt-get update' failed."

bigecho "Installing packages required for setup..."

apt-get -yq install wget dnsutils openssl iproute gawk grep sed net-tools || yum install iproute gawk grep sed net-tools wget dnsutils openssl bind-utils -y ||exiterr2
...
apt-get -yq install libnss3-dev libnspr4-dev pkg-config libpam0g-dev \
  ...
  libnss3-tools libevent-dev install ppp xl2tpd fail2ban || \
yum install audit-libs-devel bison curl-devel fipscheck-devel flex \
	...
	pam-devel pkgconfig systemd-devel unbound-devel xmlto -y|| exiterr2
...
if [ "$(packaging/utils/lswan_detect.sh init)" = "systemd" ]; then
  apt-get -yq install libsystemd-dev || yum install -y systemd-devel ||exiterr2
fi
